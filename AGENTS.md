...
Schemas are documented in `.Codex/skills/update-site-data/references/schemas.md`. If you change a schema or renderer, update that file in the same session.
...
The full listing pages (`news.html`, `papers.html`, `projects.html`, `talks.html`, `teaching.html`, `software.html`, `blog.html`, `dissertation.html`) live at the repo root — sibling to `index.html`, not nested in a subdirectory — specifically so they can reuse the same `./`-relative paths to `css/`, `js/`, and `data/` with no path-rewriting. Each loads the same renderer script(s) as the homepage, but its container(s) have no `data-limit`, so they render the full list. `about` and `contact` are always short enough that they're never truncated and have no "see all" page.
